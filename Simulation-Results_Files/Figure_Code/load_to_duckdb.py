...
# Folders to process
folder_paths = [
    '/Users/shashankpritam/github/invadego/sim_x0_b0.01',
    '/Users/shashankpritam/github/invadego/sim_x0_b0.02',
    '/Users/shashankpritam/github/invadego/sim_x0.01_b0.01',
    '/Users/shashankpritam/github/invadego/sim_x0.01_b0.02',
]
# Matches file names like "output_x0b001_-10_0.txt" capturing x, y, z values
# Capture numeric pieces (integers or decimals) for x parts plus y and z
filename_pattern = re.compile(r'output_x([\d\.]+)b([\d\.]+)_(-?\d+)_(-?\d+)\.txt')
...
def parse_file(full_file_path):
    ...
    a, b, y, z = match.groups()
    # Construct a string key for x that uniquely combines the two parts (e.g. '0_001')
    x_str = f"{a}_{b}"
    seed = None  # Will be filled once we encounter the header line containing the seed
    data = []
    ...
    with open(full_file_path, 'r') as file:
        for line in file:
            ...
            if len(parts) == len(column_names):
                try:
                    parsed_row = [x_str, int(y), int(z), int(seed) if seed is not None else None]
    ...
    df = pl.DataFrame(data, schema=total_columns, orient="row")
    return df, None

def insert_into_duckdb(dfs, db_file):
    conn = duckdb.connect(database=db_file, read_only=False)
    try:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS simulations (
                x VARCHAR, y INTEGER, z INTEGER, seed BIGINT,
...
def process_folder(folder_path: str):
    db_name = os.path.basename(folder_path) + '.duckdb'
    files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if (f.endswith('.txt') and f.startswith('output_x'))]
    ...
    # Insert data into DuckDB
        insert_into_duckdb(dfs_to_insert, db_name)
...
def main():
    for folder in folder_paths:
        if os.path.isdir(folder):
            ...
            print(f'Skipped missing folder: {folder}', file=sys.stderr)
