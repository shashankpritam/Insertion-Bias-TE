...
def get_rand_clusters(): 
    lower_limit = math.log10(1e+2) # 100 bp  # Lower bound
    upper_limit = math.log10(1e+7) # Upper bound, note the cluster size is in bp (Range is upto 10Mb)
...
def combine_output_files(output_directory, count):
    combined_file = os.path.join(output_directory, "combined.txt")
    
    def copy_file(i):
        filename = os.path.join(output_directory, f"{i}.txt")
        try:
    ...
    with open(combined_file, "wb") as outfile:
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(copy_file, i) for i in range(count)]
...
# This is the "main"
def main():
    # Assume args.threads, args.output, and args.count are defined
    submit_job_max_len(commandlist, max_processes=args.threads)
    
    # Combine the output files
    combine_output_files(args.output, args.count)
