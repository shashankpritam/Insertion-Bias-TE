#!/bin/bash
#SBATCH --job-name=reStormAll2ns
#SBATCH --output=reStormAll2ns_%A_%a.log
...
while read -r i j; do
    ...
    (
        $tool --N $N --gen $gen --genome $genome --cluster $cluster --no-x-cluins --rr $rr --rep $rep --u $u --basepop "$basepop" --steps $steps --sampleid "$sampleid" > "$folder/output_${sampleid}.txt"
    ) &
    ((count++))
