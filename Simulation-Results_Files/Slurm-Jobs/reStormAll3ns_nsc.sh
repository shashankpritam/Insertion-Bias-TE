#!/bin/bash
#SBATCH --job-name=reStormAll3ns
#SBATCH --output=reStormAll3ns_%A_%a.log
...
while read -r i j; do
    ...
    (
        $tool --N $N --gen $gen --genome $genome --cluster $cluster --no-x-cluins --rr $rr --rep $rep --u $u --x $x --basepop "$basepop" --clonal true --steps $steps --sampleid "$sampleid" > "$folder/output_${sampleid}.txt"
    ) &
    ((count++))
