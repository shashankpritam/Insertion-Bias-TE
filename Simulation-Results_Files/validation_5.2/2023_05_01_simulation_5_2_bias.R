...
df <- read.table("2023_05_01_simulation_5_2_bias", fill = TRUE, sep = "\t")
...
# Plotting 90, 70, 0, -70, -80 & -90 values of Bias Only
# library(tidyverse)
# library(RColorBrewer)
# library(ggpubr)
# theme_set(theme_bw())
...
# 
# df$phase <- factor(df$phase, levels = c("rapi", "trig", "shot", "inac"))
# df <- df %>% filter(sampleid %in% c("b90", "b70", "b0", "bm70", "bm80", "bm90"))
# df$sampleid <- factor(df$sampleid, levels = c("b90", "b70", "b0", "bm70", "bm80", "bm90"))
# 
# g <- ggplot() +
#   geom_line(data = df, aes(x = gen, y = avtes, group = rep, color = phase), alpha = 1, linewidth = 0.7) +
#   xlab("Generation") +
...
# # Remove the extra NA column
# df <- df[, !is.na(colnames(df))]
# # Filter out the desired bias values
# df <- df %>% filter(sampleid %in% c("b90", "b70", "b0", "bm70", "bm80", "bm90"))
# 
# # Divide in shot and inact phases
# df1 <- subset(df, phase %in% c("shot", "inac"))
...
# 
# # Summary statistics
# df2 <- select(df2, -c("spacer_1", "spacer_2", "spacer_3", "spacer_4"))
# 
# df_count <- df2 %>%
#   dplyr::count(sampleid, phase)
# 
